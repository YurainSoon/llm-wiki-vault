...
所有 skill 默认**跟随你输入的语言**——用中文问，表格 / 总结 / `reason` 字段就用中文回；用英文问就用英文回。要固定成某种语言、不跟随输入，**在 session 开头告诉你的 LLM，或者修改对应 `SKILL.md` 顶部的 language 指令**——这就是这套工作流默认依赖的"改你自己的 skill"循环。
